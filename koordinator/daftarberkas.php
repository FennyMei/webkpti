...
<thead>
  ...
  $sql = mysql_query("SELECT * FROM storage");
  $no =1;
...
while($data = mysql_fetch_assoc($sql)){
 echo '
  <tr>
      ...
      if($data['lokasi']!=NULL){
        echo '<td><a href="'.$data['lokasi'].'">'.$data['file'].'</a></td>';
      }
      else{
        echo '<td><a href="'.$data['link'].'">'.$data['file'].'</a></td>';
      }
      ?>
      <td><div><p><?php echo ($data['file_desc']); ?></p></div></td>
      <?php
      echo "<td><a href='deletefile.php?id=".$data['id_file']."'class='btn btn-danger' onclick='return confirmHapus()'><button>Hapus</button></a></td></font></td>";
